...
select P.nome as 'Produto', P.id as 'Id', OP.valor_unit as 'Valor', SUM(OP.valor_total_item) as 'OrcamentoSetembro2014', P.saldo as 'Estoque', O.id as 'Orcamento'
from orcamentos_produtos OP
...
-- acréscimo de 20% nos produtos com menos de 5 unidades ou menos em estoque

select Id, Produto, Valor, SUM(Valor*1.20) as Valor_mais20 from top_10_produtos_setembro_2014 where Estoque <= 5;
...
-- GROUP BY
-- O Group By é utilizado quando existe uma função de agregação que necessita de agrupamento no select, como obter o total de todos os produtos vendidos, por produto

select P.id as 'Id Produto', P.nome as 'Produto', SUM(OP.valor_total_item) as 'Valor total vendido do produto'
from orcamentos_produtos OP
	inner join produtos P on P.id = OP.id_prod
    inner join orcamentos O on O.id = OP.id_orc
group by P.id;
...
-- HAVING
-- O Having serve como um WHERE específico ao Group By no select, ou uma função de agregação dentro deste, como a quantia vendida de um produto específico em um período específico
-- Aqui, a quantia de Computadores vendidos entre 2014 e 2017

select P.nome as 'Produto', SUM(OP.qtd) as 'Quantia vendida'
from orcamentos_produtos OP
	inner join produtos P on P.id = OP.id_prod
    inner join orcamentos O on O.id = OP.id_orc
where O.dataCriado between '2014-01-01' and '2017-01-01'
group by P.id
having P.nome LIKE 'Computador %';

-- UNION
-- O Union serve para unir resultados de um SELECT em uma única lista, como por exemplo, uma lista que possui todos os IDs, tanto de produtos quanto de Orçamentos
-- O ALL é utilizado para que, mesmo na existência de duplicatas, que ocorrerá uma vez que id produto vai de 1 - 10 e id orcamento vai de 1 - 30, TODOS os resultados sejam exibidos

select id, nome as 'Produto e Status' from produtos union all select id, statusOrc from orcamentos;

-- LEFT JOIN
-- O Left Join serve como o Inner Join, mas onde este apenas realiza a união onde os resultados possuem relacionamento, aquele retorna a tabela ESQUERDA (a primeira inserida) completa,  
-- e as ocorrências da(s) tabela(s) à DIREITA caso existam alguma, do contrário, devolverá null na "relação" entre esquerda e direita, foi utilizado no exemplo para apagar os produtos
-- que não haviam sido orçados, onde eram batidos os produtos com seus orçamentos, caso o produto não possuísse orçamento ( is null ) o produto seria apagado.
