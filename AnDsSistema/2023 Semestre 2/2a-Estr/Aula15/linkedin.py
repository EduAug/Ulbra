...
def excluir_contato(cont_id):
    ...
    try:
        cursor.execute('''
            SELECT id FROM conexoes WHERE contact_id1 = ? OR contact_id2 = ?
        ''',(cont_id,cont_id))

        found_in_relation = cursor.fetchone()

        if found_in_relation:

            print("Não é possível excluir um usuário que está em uma relação")
        else:
        ...
        conn.rollback()
        print("Erro ao excluir/Id não encontrado")
